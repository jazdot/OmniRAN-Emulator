...
func handleTargetAccessTrigger(ue *context.GNBUe, message []byte, gnb *context.GNBContext) bool {
	...
	if len(message) == 10 && message[0] == 0x00 && message[1] == 0x04 {
		...
		gnb.RangeUePool(func(ranUeId int64, temp *context.GNBUe) bool {
			if temp.GetAmfUeId() == amfUeId || temp.GetSourceAmfUeId() == amfUeId {
				realUe = temp
				return false
			}
			return true
		...
		// Send AMF UE ID update to UE so it updates its context to the target one
		if realUe.GetAmfUeId() != amfUeId {
			log.Infof("[GNB-Target] Updating UE AMF UE ID from source ID %d to target ID %d", amfUeId, realUe.GetAmfUeId())
			updateMsg := make([]byte, 10)
			updateMsg[0] = 0x00
			updateMsg[1] = 0x03
			binary.BigEndian.PutUint64(updateMsg[2:10], uint64(realUe.GetAmfUeId()))
			_, err := realUe.GetUnixSocket().Write(updateMsg)
			if err != nil {
				log.Errorf("[GNB-Target] Error writing AMF UE ID update to UE socket: %v", err)
			}
		}
